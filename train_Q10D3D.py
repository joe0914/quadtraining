...
dataset = dataio.Quadcopter10DP3D(numpoints=65000,
...
model = modules.SingleBVPNet(in_features=14, out_features=1, type=opt.model, mode=opt.mode, #What is in feature vs out feature?
                             final_layer_factor=1., hidden_features=opt.num_nl, num_hidden_layers=opt.num_hl)
model.cuda()

#Define the loss
loss_fn = loss_functions.initialize_hji_Q10DP3D(dataset, opt.minWith, opt.diffModel_mode)
